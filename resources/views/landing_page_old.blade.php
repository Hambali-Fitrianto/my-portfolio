...
<body class="bg-lightBg text-slate-700 dark:bg-darkBg dark:text-gray-300 antialiased font-sans relative">
                ...
                <div>
                    <div class="relative aspect-video w-full overflow-hidden bg-black/50 border-b border-slate-200/10 dark:border-white/5 group/imgContainer">
                        ...
                        <button type="button" onclick="openLightboxModal('{{ asset('storage/projects/' . $project->images->first()->path_gambar) }}')" 
                            class="w-full h-full block cursor-pointer overflow-hidden relative z-10" title="Perbesar Gambar Project">
                ...
                <div class="p-6 pt-0 space-y-4 mt-auto">
                    ...
                    <div class="grid grid-cols-2 gap-3 pt-3 border-t border-slate-200 dark:border-white/5">
                        ...
                        <a href="{{ $project->link_project }}" target="_blank"
                            class="w-full text-center py-2.5 px-3 bg-blue-600 hover:bg-blue-700 text-white text-xs font-extrabold rounded-xl transition flex items-center justify-center gap-1.5 shadow-md shadow-blue-500/20 group/btn relative z-20"
                        ...
                        <button type="button" disabled
                            class="w-full py-2.5 px-3 bg-slate-200 dark:bg-white/[0.02] text-slate-400 dark:text-gray-600 text-xs font-bold rounded-xl border border-dashed border-slate-300 dark:border-white/5 cursor-not-allowed flex items-center justify-center gap-1 relative z-20"
    ...
    <div id="portfolioModal" class="fixed inset-0 z-40 hidden items-center justify-center p-4 bg-black/80 backdrop-blur-sm opacity-0 transition-opacity duration-300">
    ...
    <div id="lightboxModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-black/90 backdrop-blur-md opacity-0 transition-opacity duration-300">
        <div class="relative max-w-4xl max-h-[85vh] transform scale-95 transition-transform duration-300 flex flex-col items-center">
            <button type="button" onclick="closeLightboxModal()" class="absolute -top-12 right-0 md:right-4 text-white hover:text-gray-300 bg-white/10 hover:bg-white/20 w-10 h-10 rounded-xl flex items-center justify-center transition cursor-pointer text-sm">
                <i class="fas fa-times"></i>
            </button>
            <img id="lightboxImg" src="" class="max-w-full max-h-[75vh] rounded-2xl object-contain shadow-2xl border border-white/10">
        </div>
    </div>
    ...
    <script>
        ...
        // Lightbox Modal Handlers (Profil & Project Image)
        ...
        function closeLightboxModal() {
            const lightbox = document.getElementById('lightboxModal');
            const htmlElement = document.documentElement;
            const modalDetail = document.getElementById('portfolioModal');
            ...
            setTimeout(() => {
                ...
                // PERBAIKAN LOGIC SCROLL LOCK: Hanya buka scroll jika modal detail tidak aktif
                if (document.getElementById('mobile-menu').classList.contains('hidden') && modalDetail.classList.contains('hidden')) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.addEventListener('resize', function() {
                if (window.innerWidth >= 768) {
                    mobileMenu.classList.replace('flex', 'hidden');
                    menuIcon.className = 'fas fa-bars';
                    if (modal.classList.contains('hidden') && document.getElementById('lightboxModal').classList.contains('hidden')) {
                        htmlElement.classList.remove('overflow-hidden');
                    }
                }
            });
            ...
            // PERBAIKAN INTEGRASI: Event listener tombol detail dikembalikan ke kestabilan murni
                ...
                btn.addEventListener('click', function(e) {
                    ...
                    if (images && images.length > 0) {
                        ...
                        images.forEach(src => {
                            ...
                            item.className = 'relative aspect-video rounded-xl overflow-hidden border border-slate-200 dark:border-white/5 bg-black shadow-inner group/subImg cursor-pointer';
                            // PERBAIKAN: Memastikan struktur button di dalam modal aman & bisa memicu Lightbox modal
                            item.innerHTML = `
                                <button type="button" class="w-full h-full block absolute inset-0 z-30" onclick="openLightboxModal('${src}')"></button>
                                <img src="${src}" class="w-full h-full object-cover relative z-10" onerror="this.src='https://placehold.co/600x400/222/555?text=Preview+Error'">
                                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover/subImg:opacity-100 flex items-center justify-center transition-opacity duration-300 z-20">
            ...
            function closeModal() {
                ...
                setTimeout(() => {
                    ...
                    if (mobileMenu.classList.contains('hidden') && document.getElementById('lightboxModal').classList.contains('hidden')) {
                        htmlElement.classList.remove('overflow-hidden');
            ...
            // Event Area Luar Lightbox & Global ESC Key
            const lightbox = document.getElementById('lightboxModal');
            ...
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape') {
                    if (!lightbox.classList.contains('hidden')) closeLightboxModal();
